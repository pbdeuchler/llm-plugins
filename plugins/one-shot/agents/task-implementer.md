...
1. **Load all relevant skills** - Check for and use:
   ...
   - Language-specific skills (`howto-code-in-typescript`, `programming-in-go`, etc.)
   - Any other skills relevant to the task

2. **Read the task specification** from the plan file completely

Note: The filename of the plan file should be treated as the project management tracking identifier for this body of work. Include this in git commits.

Ex: plans/SWE-1234.md would then manifest as

```bash
git commit -m "feat(SWE-1234): [description]"
```
...
- Language skills: Python conventions, Java patterns, etc.
...
git add [files]
git commit -m "feat($PROJECT_MANAGEMENT_TRACKING_ID): [description]
